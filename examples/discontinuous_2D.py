...
def f(x):
    ...
    c = 10./(2**np.linspace(1,N,N))
    
    Z      = exp(np.sum(c*x,axis=1))
    idx    = np.logical_or(np.greater(x[:,0], 0.5),np.greater(x[:,1], 0.5))    
    Z[idx] = 0
...
main_node = me_leja(f,discontinuous=True,dist=jpdf,p_max=p_max)
